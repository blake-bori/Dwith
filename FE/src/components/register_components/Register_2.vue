  ...
  <div>
    ...
    <div class="row register-input2">
      <div class="col-4 col-md-3 text-center" v-for="job in jobList" :key="job">
        <input type="radio" :id="job" name="jobList" :value="job" v-model="registerInterestJob" />
...
export default {
  data() {
    return {
      registerInterestJob: "",
      jobList: ["경영/사무", "IT/인터넷", "디자인", "흥미4", "흥미5", "흥미6", "흥미7", "흥미8", "흥미9", "흥미10", "기타"],
    };
  },
  methods: {
    // 이전 단계로 이동
    ...
    // 다음 단계로 이동
    nextStep() {
      if (this.registerInterestJob.length === 0) alert("직업 선택 필요");
      else {
        // vuex에 정보 저장 후 다음 단계로 이동
        this.$store.state.auth.registerInfo.registerInterest.job = this.registerInterestJob;
        this.$emit("nextStep");
      }
    },
  },
  ...
  mounted() {
    ...
    this.registerInterestJob = this.$store.state.auth.registerInfo.registerInterest.job;
  },
};
</script>
