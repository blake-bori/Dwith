  ...
  <div>
    ...
    <div class="register-input3">
      ...
      <div style="margin-bottom: 10px">
        {{ registerInterest.job }}
      </div>
      <!-- 관심 직종 세부 분야(check) -->
      <div>
        <div class="d-inline-block mr-1 job-tag" v-for="tag in jobTagLists" :key="tag">
          <input type="checkbox" :id="tag" name="jobList" :value="tag" @change="checkList(tag)" v-model="registerInterest.detailJob" />
...
export default {
  ...
  methods: {
    ...
    nextStep() {
      ...
      if (this.registerInterest.detailJob.length > 0) {
        this.$store.state.auth.registerInfo.registerInterest.detailJob = this.registerInterest.detailJob;
        this.$emit("nextStep");
      } else alert("세부 직무 선택 필요");
    },
    // 세부 직무 3개 이상 선택했을 시 선택 취소시킴
    checkList(tag) {
      if (this.registerInterest.detailJob.length > 3) {
        alert("최대 3개까지 선택 가능");
        this.registerInterest.detailJob.pop();
  ...
  mounted() {
    ...
    this.registerInterest.job = this.$store.state.auth.registerInfo.registerInterest.job;

    // 해당 interest에 대한 세부 직무 데이터를 서버로부터 불러옴
    this.jobTagLists = this.$store.state.auth.registerSubJobTag[this.registerInterest.job];
